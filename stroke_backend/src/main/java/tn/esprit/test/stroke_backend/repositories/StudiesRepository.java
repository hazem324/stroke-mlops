...
public interface StudiesRepository extends JpaRepository<Studies, Long> {
    ...
    List<Studies> findAllByPatientIdAndPatientDoctor(
            Long patientId,
            User doctor
    );

    // Une étude appartenant à un patient du médecin
    Optional<Studies> findByIdAndPatientDoctor(
            Long studyId,
            User doctor
    );

    // Toutes les études d'un patient
    List<Studies> findByPatientId(
            Long patientId
    );

    // Une étude appartenant à un patient
    Optional<Studies> findByIdAndPatientId(
            Long studyId,
            Long patientId
    );

    // Vérifie si un studyCode existe déjà
    boolean existsByStudyCode(
            String studyCode
    );
}